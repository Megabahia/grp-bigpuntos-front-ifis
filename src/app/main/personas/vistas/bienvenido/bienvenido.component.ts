...
export class BienvenidoComponent implements OnInit, OnDestroy {
    ...
    obtenerProductos() {
        const subsObtenerProductos = this._bienvenidoService.obtenerProductos(
            {
                tipo: 'presentacion',
            }
        ).subscribe((valor) => {
    ...
    modalOpenVC(modalVC, id) {
        const subsObtenerProducto = this._bienvenidoService.obtenerProducto(id).subscribe((valor) => {
            this.producto = valor;
